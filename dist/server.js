...
import { addUserToRoom, removeUserFromRoom, getOnlineUsers, isRateLimited, publishToChannel, subscribeToChannel, } from './utils/redis.js';
        ...
        try {
            ...
            switch (message.type) {
                case 'join': {
                    ...
                    const user = await chatService.addUser(connectionId, userId, message.room || 'general');
                    ...
                    const onlineUsers = await Promise.all(onlineUserIds.map(async (id) => {
                    ...
                    broadcastToRoom(user.roomId, { type: 'userJoined', username: user.username }, ws);
                    break;
                }
                case 'message': {
                    ...
                    const savedMessage = await chatService.addMessage(message.text, user.id, user.roomId);
                ...
                case 'getOnlineUsers': {
                    ...
                    if (user) {
                        ...
                        ws.send(JSON.stringify({ type: 'onlineUsers', userIds: onlineUsers }));
                    }
                    break;
                }
            }
